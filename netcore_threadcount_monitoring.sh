...
# Invoke if the script receives SIGINT | SIGTERM signals
terminate_script() {
  die "The script has been stopped either by SIGINT or SIGTERM signal..." 0
}
...
runtime_counter_log_file="dotnet-runtime-counter-log-$instance.csv"

# name of the lock file for generating memdump
dump_lock_file="dump_taken.lock"

# Monitor the .NET process by starting the dotnet-counters collect command in background
nohup /tools/dotnet-counters collect --process-id "$pid" --counters System.Runtime --output "$runtime_counter_log_file" &
    ...
    if [[ $line == *"ThreadPool Thread Count"* ]]; then
      ...
      if [[ "$thread_count" -gt "$threshold" ]]; then
        if [[ ! -e "$dump_lock_file" ]]; then
          ...
          nohup /tools/dotnet-dump collect -p "$pid" >> "$output_file" &
        fi
      fi
    fi
  done
